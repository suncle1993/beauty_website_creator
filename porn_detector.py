...
@author: Chen Liang
@function: 色情图像检测，可以区分清纯（非色情）和性感（色情）
"""
...
class PornDetector(object):
    ...
    def __init__(self, path_or_image):
        ...
        # output决策的三个参数
        self.skin_region_nums = None  # 皮肤区域个数
        self.skin_pixel_num_rate = None  # 皮肤区域与整个图像的比值
        self.skin_region_area_rate = None  # 最大皮肤区域与总皮肤面积的比例
    ...
    def resize(self, max_width=1000, max_height=1000):
        ...
        如果没有变化返回 0
        原宽度大于 max_width 返回 1
        原高度大于 max_height 返回 2
        原宽高大于 max_width, max_height 返回 3
        ...
        if max_width:
            if self.width > max_width:
                ...
                height_size = int((self.height * width_percent))
                file_name = self.image.filename
                # Image.LANCZOS 是重采样滤波器，用于抗锯齿
                self.image = self.image.resize((max_width, height_size), Image.LANCZOS)
                self.image.filename = file_name
                self.width, self.height = self.image.size
                self.total_pixels = self.width * self.height
                ret += 1
        if max_height:
            if self.height > max_height:
                ...
                file_name = self.image.filename
                self.image = self.image.resize((width_size, max_height), Image.LANCZOS)
                self.image.filename = file_name
                self.width, self.height = self.image.size
                self.total_pixels = self.width * self.height
                ret += 2
    ...
    def parse(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                b = pixels[x, y][2]   # blue
                # 判断当前像素是否为肤色像素
                is_skin_pixel = True if self._classify_skin(r, g, b) else False
                # 给每个像素分配唯一 id 值（1, 2, 3...height*width）
                # 注意 x, y 的值从零开始
                _id = x + y * self.width + 1
                # 为每个像素创建一个对应的 Skin 对象，并添加到 self.skin_map 中
                self.skin_map.append(self.SKIN(_id, is_skin_pixel, None, x, y))
                # 若当前像素不为肤色像素，跳过此次循环
                if not is_skin_pixel:
                    continue
    ...
    def _analyse_regions(self):
        ...
        self.skin_region_nums = len(self.skin_regions)  # 皮肤区域个数
        self.skin_pixel_num_rate = total_skin / self.total_pixels * 100  # 皮肤区域与整个图像的比值
        self.skin_region_area_rate = len(self.skin_regions[0]) / total_skin * 100  # 最大皮肤区域与总皮肤面积的比例

    def judge(self):
        # 如果皮肤区域小于 3 个，不是色情
        if self.skin_region_nums < 3:
            self.message = "Less than 3 skin regions ({_skin_regions_size})".format(
                _skin_regions_size=self.skin_region_nums)
            self.result = False
            return self.result

        # 如果皮肤区域与整个图像的比值小于 15%，那么不是色情图片
        if self.skin_pixel_num_rate < 15:
            self.message = "Total skin percentage lower than 15 ({:.2f})".format(self.skin_pixel_num_rate)
            self.result = False
            return self.result

        # 如果最大皮肤区域小于总皮肤面积的 45%，不是色情图片
        if self.skin_region_area_rate < 45:
            self.message = "The biggest region contains less than 45 ({:.2f})".format(self.skin_region_area_rate)
            self.result = False
            return self.result

        # 皮肤区域数量超过 60个，不是色情图片
        if self.skin_region_nums > 60:
            self.message = "More than 60 skin regions ({})".format(self.skin_region_nums)
            self.result = False
            return self.result
    ...
    def _classify_skin(self, r, g, b):
        ...
        ycbcr_classifier = 80 <= cb <= 120 and 133 <= cr <= 173
    ...
    def show_skin_regions(self):
        ...
        for pixel in self.skin_map:
            if pixel.id not in skin_id_set:
                src_image_data[pixel.x, pixel.y] = 0, 0, 0
            else:
                src_image_data[pixel.x, pixel.y] = 255, 255, 255
        # 源文件绝对路径
        ...
        file_name, file_ext_name = os.path.splitext(file_full_name)
        # 保存图片
        src_image.save('{}{}_{}{}'.format(
            file_directory, file_name, 'Nude' if self.result else 'Normal', file_ext_name
        ))
